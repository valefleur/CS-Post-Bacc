...
#TODO: how to make it report the garden only once? UNIQUE?  No, that's for creation, not searching...
...
#NOT the gardeners who have 1 plot total, and it happens to be at THills
...
#Find all gardeners and the location of their community garden, sorted by location
SELECT cg.location, g.lname, g.fname FROM gardener g
INNER JOIN community_garden_with_gardener cgwg ON g.gardener_id = cgwg.gardener_id
INNER JOIN community_garden cg ON cgwg.community_id = cg.community_id
ORDER BY cg.location;